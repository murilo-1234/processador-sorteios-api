    ...
    <body>
            ...
            <div class="content">
                ...
                <div class="actions-section">
                    ...
                    <div class="actions-grid">
                        ...
                        <button class="btn primary" onclick="forcarQRCode()">📱 Forçar QR Code</button>
                        <button class="btn" onclick="atualizarStatus()">📊 Atualizar Status</button>
                    </div>
                    
                    <!-- Botões diretos para casos extremos -->
                    <div style="margin-top: 15px; text-align: center;">
                        <a href="/api/reset-whatsapp" target="_blank" class="btn danger" style="text-decoration: none; margin-right: 10px;">
                            🚨 Reset Direto
                        </a>
                        <a href="/api/force-qr" target="_blank" class="btn primary" style="text-decoration: none;">
                            🔄 Forçar QR Direto
                        </a>
                        <small style="display: block; margin-top: 5px; color: #666;">
                            Use estes links se os botões acima não funcionarem
                        </small>
                    </div>
                </div>
        ...
        <script>
            ...
            async function forcarQRCode() {
                try {
                    showLoading(true);
                    showAlert('Forçando geração de QR Code...', 'info');
                    
                    const response = await fetch('/api/force-qr', {
                        method: 'GET'
                    });
                    ...
                    if (response.ok && data.success) {
                        showAlert('QR Code gerado com sucesso! Acesse /qr para escanear.', 'success');
                        ...
                        showAlert('Erro ao gerar QR Code: ' + (data.error || data.message || 'Erro desconhecido'), 'error');
                    }
                    ...
                    showAlert('Erro ao forçar QR Code', 'error');
                    showLoading(false);
                }
            }
        </script>

...
router.get('/reset-whatsapp', async (req, res) => {
  try {
    ...
    if (qrGenerated) {
      ...
      const tried = await whatsappClient.tryPairingIfConfigured?.();
      if (tried) {
        return res.json({ success: true, message: 'Pairing code gerado. Acesse /code', action: 'pairing_ready', qrAvailable: false, pairingAvailable: true });
      }
      res.json({ 
        success: true, 
...
router.get('/debug/whatsapp', async (req, res) => {
  try {
    const wa = req.app.locals.whatsappClient;
    if (!wa) return res.status(503).json({ error: 'whatsapp client not ready' });
    return res.json(wa.getDebug ? wa.getDebug() : { error: 'getDebug not available' });
  } catch (e) {

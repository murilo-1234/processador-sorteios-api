...
router.get('/status', async (req, res) => {
  try {
    ...
    const status = {
      ...
      jobs: {},
      jobScheduler: {
        enabled: !!jobScheduler,
        status: jobScheduler ? 'active' : 'disabled'
      },
      timestamp: new Date().toISOString(),
      uptime: process.uptime()
    };

    // Jobs info (apenas se jobScheduler existir)
    if (jobScheduler && jobScheduler.jobs) {
      ...
      status.jobs.info = 'Job scheduler desabilitado - focando na detecção de desconexão';
    }

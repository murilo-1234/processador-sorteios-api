...
class WhatsAppClient extends EventEmitter {
  ...
  setupEventListeners(saveCreds) {
    ...
    this.sock.ev.on('connection.update', async (update) => {
      ...
      if (!this.isConnected && !this.sock.authState.creds.registered && this.shouldUsePairing()) { await this.requestPairingCode(); }
      
      // Fallback para QR code se pairing code falhar
      if (qr && !this.qrCodeGenerated && !this.currentPairingCode) {
        logger.info('📱 QR Code gerado como fallback');
      ...
      if (connection === 'close') {
        ...
        this.currentRetry = 0;
        this.currentQRCode = null; // Limpar QR Code quando conectado
        this.currentPairingCode = null; // Limpar pairing code quando conectado
  ...
  /**
   * Solicitar pairing code para autenticação
   */
  async requestPairingCode() {
    try {
      ...
      // Gerar número de telefone fictício para pairing (necessário para Baileys)
      const phoneNumber = (process.env.WHATSAPP_PHONE_NUMBER || '').trim();
      if (!phoneNumber) { throw new Error('WHATSAPP_PHONE_NUMBER ausente: defina no .env ou use QR via /qr'); }
      ...
      logger.info('✅ Pairing code gerado com sucesso!');
      console.log('\n🔗 PAIRING CODE PARA WHATSAPP:\n');
      console.log(`📱 Código: ${code}`);
      console.log('\n💡 COMO USAR:');
      console.log('1. Abra o WhatsApp no seu celular');
      console.log('2. Vá em: Aparelhos Conectados');
      console.log('3. Toque em: "Conectar com código"');
      console.log(`4. Digite: ${code}`);
      console.log('5. Pronto! WhatsApp conectado.\n');
  ...
  async clearSession() {
    ...
    this.qrCodeGenerated = false;
    this.currentQRCode = null; // Limpar QR Code atual
    this.currentPairingCode = null; // Limpar pairing code atual
    this.currentRetry = 0;
    this.resetCircuitBreaker();
  }
}

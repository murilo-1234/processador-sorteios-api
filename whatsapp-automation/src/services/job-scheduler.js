...
class JobScheduler {
  ...
  async initialize() {
    ...
    try {
      ...
      // Registrar job de monitoramento contínuo de sorteios
      this.scheduleJob('monitor-sorteios', {
        schedule: process.env.CRON_SCHEDULE_MONITOR || '5,35 * * * *',
  ...
  /**
   * Handler do job de monitoramento contínuo de sorteios
   */
  async handleMonitoramentoSorteios(executionId) {
    logger.info('🔍 Iniciando monitoramento de sorteios...');
    ...
    await sorteiosModule.monitorarSorteiosElegiveis(executionId);
    
    logger.info('✅ Monitoramento de sorteios concluído');
  }

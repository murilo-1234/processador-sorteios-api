...
zonedTimeToUtcSafe = (date) => {
    ...
    **return new Date(d.getTime() + Math.abs(TZ\_OFFSET\_MINUTES) \* 60 \* 1000);**
...
try {
    ...
    **const offsetCfg = Math.abs(Number(process.env.TZ\_OFFSET\_MINUTES || -180));**

    **const offsetLocal = Math.abs(new Date().getTimezoneOffset());**

    **if (tzEnv.includes('sao\_paulo') || tzEnv.includes('são\_paulo')) return true;**
...
if (!targetJids.length) {
    ...
    **return { ok: false, reason: 'no\_target\_groups' };**
...
const H_IMG  = findHeader(headers, [
    **'url\_imagem\_processada','url\_imagem\_sorteio','imagem\_sorteio','url\_imagem','imagem','image\_url'**
...
if (!H_ID || !H_DATA || !H_HORA || !H_PROD || !H_IMG || !H_PLAN) {
    **throw new Error(**
      ...
      **`Obrigatórios: id | data | (horario/hora) | produto | url\_imagem\_processada | url\_planilha.`**
...
if (!sock && !dryRun) {
    ...
    **return { ok: false, reason: 'wa\_disconnected' };**
...
for (let i = 0; i < items.length; i++) {
    ...
    **const id      = coerceStr(row\[H\_ID]);**

    **const product = coerceStr(row\[H\_PROD]);**

    **const dateStr = coerceStr(row\[H\_DATA]); // dd/MM/yyyy**

    **const horaStr = coerceStr(row\[H\_HORA]);**

    **const imgUrl  = coerceStr(row\[H\_IMG]);**

    **const planUrl = coerceStr(row\[H\_PLAN]);**
    ...
    **if (!imgUrl)  missing.push('url\_imagem\_processada');**

    **if (!planUrl) missing.push('url\_planilha');**



    **if (missing.length) { skipped.push({ row: rowIndex1, id, reason: 'faltando\_campos', missing }); continue; }**
    ...
    **if (spDate < todayLocalDateOnly) { skipped.push({ row: rowIndex1, id, reason: 'past\_draw' }); continue; }**

    **if (hasResultForRow(row, hdrs))  { skipped.push({ row: rowIndex1, id, reason: 'has\_result' }); continue; }**



    **const p1Posted   = String(row\[H\_P1] || '').toLowerCase() === 'postado';**

    **const p2Posted   = String(row\[H\_P2] || '').toLowerCase() === 'postado';**

    **const p1Canceled = isCanceledFlag(row\[H\_P1]);**

    **const p2Canceled = isCanceledFlag(row\[H\_P2]);**



    **const dayLocal    = mkLocalDateAtHour(spDate, PROMO\_POST\_HOUR);**

    **const beforeLocal = new Date(dayLocal.getTime() - PROMO\_BEFORE\_DAYS \* 24 \* 60 \* 60 \* 1000);**
    ...
    **if (!p1Canceled \&\& now >= p1At) {**
      ...
      **const alreadyP1 = parseGroups(row\[H\_P1G]);**

      **if (isSuperset(alreadyP1, targetSet)) {**

        **if (!p1Posted \&\& !dryRun) {**

          **await updateCellByHeader(sheets, spreadsheetId, tab, headers, rowIndex1, H\_P1, 'Postado');**

          **await updateCellByHeader(sheets, spreadsheetId, tab, headers, rowIndex1, H\_P1AT, new Date().toISOString());**
        ...
        **try {**
          ...
          **} else {**
            ...
            **for (const rawJid of targetJids) {**
              ...
              **const opts = BAILEYS\_LINK\_PREVIEW\_OFF ? { linkPreview: false } : undefined;**
              ...
              **await updateCellByHeader(sheets, spreadsheetId, tab, headers, rowIndex1, H\_P1G, groupsToCell(alreadyP1));**
            ...
            **if (anySent \&\& isSuperset(alreadyP1, targetSet)) {**
              ...
              **await updateCellByHeader(sheets, spreadsheetId, tab, headers, rowIndex1, H\_P1AT, ts);**
    ...
    **if (!p2Canceled \&\& now >= p2At) {**
      ...
      **const alreadyP2 = parseGroups(row\[H\_P2G]);**

      **if (isSuperset(alreadyP2, targetSet)) {**

        **if (!p2Posted \&\& !dryRun) {**

          **await updateCellByHeader(sheets, spreadsheetId, tab, headers, rowIndex1, H\_P2, 'Postado');**

          **await updateCellByHeader(sheets, spreadsheetId, tab, headers, rowIndex1, H\_P2AT, new Date().toISOString());**
        ...
        **try {**
          ...
          **} else {**
            ...
            **for (const rawJid of targetJids) {**
              ...
              **await updateCellByHeader(sheets, spreadsheetId, tab, headers, rowIndex1, H\_P2G, groupsToCell(alreadyP2));**
            ...
            **if (anySent \&\& isSuperset(alreadyP2, targetSet)) {**
              ...
              **await updateCellByHeader(sheets, spreadsheetId, tab, headers, rowIndex1, H\_P2AT, ts);**

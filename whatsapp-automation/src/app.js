...
class App {
  ...
  async setupRoutes() {
    ...
    // Inicialização manual do WA + debug
    this.app.post('/api/whatsapp/init', async (req, res) => {
      try {
        if (!this.whatsappClient) this.whatsappClient = new WhatsAppClient();
        await this.whatsappClient.initialize();
        res.json({ ok: true, debug: this.whatsappClient.getDebug ? this.whatsappClient.getDebug() : {} });
      } catch (e) {
        res.status(500).json({ ok: false, error: e?.message, debug: this.whatsappClient?.getDebug ? this.whatsappClient.getDebug() : {} });
      }
    });
  ...
  async initializeWhatsApp() {
    ...
    const ensure = async () => {
      try {
        ...
        logger.error('❌ Falha ao inicializar WhatsApp (retry em 5s):', e?.message || e);

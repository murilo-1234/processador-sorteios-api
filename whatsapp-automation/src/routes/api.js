...
// Debug de filesystem do path da sessão
router.get('/debug/fs', async (req, res) => {
  try {
    const wa = req.app.locals.whatsappClient;
    const p = wa?.sessionPath || process.env.WHATSAPP_SESSION_PATH || '/tmp/whatsapp-session';
    const fs = require('fs');
    const stat = (() => { try { return fs.statSync(p); } catch(e) { return { error: e?.message }; } })();
    const exists = !stat.error;
    const files = (() => { try { return fs.readdirSync(p); } catch(e) { return ['<inacessível>']; } })();
    // tenta escrever um arquivo
    let writeOk = false, writeErr = null;
    try { fs.writeFileSync(p + '/.__writeprobe', String(Date.now())); writeOk = true; fs.rmSync(p + '/.__writeprobe', {force:true}); } catch(e) { writeErr = e?.message; }
    return res.json({ path: p, exists, stat, files, writeOk, writeErr });
  } catch (e) {

...
// === Session backup (opcional) ===
let sessionBackup = null;
try {
  sessionBackup = require('./services/session-backup'); // deve ser tolerante à ausência
} catch (_) {}
...
class App {
  constructor() {
    ...
    this.isFallbackEnabled =
      envOn(process.env.WA_CLIENT_AUTOSTART, false) || envOn(process.env.WA_FALLBACK_ENABLED, false);
    ...
    // (NOVO) destinos de postagem ad-hoc por env (não interfere no restante do sistema)
    this.postToJids = [...new Set(parsePhonesToJids(process.env.WA_POST_TO))];
  ...
  routes() {
    ...
    this.app.post('/api/whatsapp/disconnect', async (_req, res) => {
      try {
        ...
        // (NOVO) backup de sessão antes de apagar, se habilitado e serviço disponível
        if (envOn(process.env.SESSION_BACKUP, false) && sessionBackup) {
          for (const d of SESSION_DIR_CANDIDATES) {
            try {
              if (typeof sessionBackup.backup === 'function') {
                await sessionBackup.backup(d);
              } else if (typeof sessionBackup.backupDir === 'function') {
                await sessionBackup.backupDir(d);
              }
            } catch (e) {
              console.warn('[session-backup] falha ao backup:', d, e?.message || e);
            }
          }
        }
        ...
        sseBroadcast('default', {
          type: 'status',
          payload: { ok: true, connected: false, connecting: false, hasSock: false },
        });
    ...
    this.app.get('/api/groups/sync', async (_req, res) => {
      try {
        ...
        return res
          .status(503)
          .json({ ok: false, error: 'Sem sessão disponível (admin desconectado e fallback desabilitado).' });
      } catch (e) {
    ...
    this.app.post('/api/groups/test-post', async (_req, res) => {
      try {
        ...
        // alvos: seleção do painel OU (NOVO) WA_POST_TO se existir (sem interferir no painel)
        let targets =
          (Array.isArray(st.postGroupJids) && st.postGroupJids.length
            ? st.postGroupJids
            : st.resultGroupJid
            ? [st.resultGroupJid]
            : []) || [];
        if ((!targets || !targets.length) && this.postToJids.length) {
          targets = this.postToJids; // apenas se painel não definiu nada
        }
        if (!targets.length) {
          return res.status(400).json({ ok: false, error: 'Nenhum grupo/destino selecionado' });
        }
    ...
    this.app.post('/api/alerts/test', async (_req, res) => {
      if (!this.alertCfg.enabled) {
        return res.status(400).json({
          ok: false,
          error: 'Defina ALERT_WA_PHONES/ALERT_WA_PHONE (ou ALERT_WA_JID[S]) para habilitar alertas.',
        });
      }
  ...
  async afterListen() {
    ...
    const sess = await hasSavedSession();
    console.log(
      `🚀 Boot info -> Fallback: ${this.isFallbackEnabled ? 'ON' : 'OFF'} | sessionDir=${sess.dir || '(none)'} | files=${sess.files}`
    );
        ...
        try {
          ...
          if (!connected && this.alertCfg.enabled && !this.alertState.downNotifiedAt) {
            ...
            if (elapsed >= this.alertCfg.graceMs) {
              const ok = await this.sendAlert(
                '⚠️ WhatsApp (admin) está offline há alguns minutos. Tentando reconectar automaticamente.'
              );

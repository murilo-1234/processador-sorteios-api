...
class WhatsAppClient extends EventEmitter {
  ...
  async initialize() {
    ...
    try { fs.mkdirSync(this.sessionPath, { recursive: true }); const p = `${this.sessionPath}/.__rwtest`; fs.writeFileSync(p, String(Date.now())); fs.rmSync(p, {force:true}); } catch(e){ this.lastInitError = e?.message; logger.error('❌ Sessão sem permissão de escrita:', e?.message); throw e; }
...
getDebug() {
  try {
    const fs = require('fs');
    let files = [];
    try { files = fs.readdirSync(this.sessionPath); } catch(e) { files = ['<inacessível>']; }
    return {
      ...
      isConnected: !!this.isConnected,
      qrCodeGenerated: !!this.qrCodeGenerated,
      lastQRAt: this.lastQRAt || null,
      lastPairingAt: this.lastPairingAt || null,
      hasSock: !!this.sock,
      files,
      lastUpdate: this.lastUpdate || null,
      lastErrors: Array.isArray(this.lastErrors) ? this.lastErrors.slice(-5) : []
    };
  } catch (e) {
